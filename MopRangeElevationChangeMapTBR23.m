% MopRangeElevationChangeMapTBR23.m
% 
% Creates a 2D map of elevation changes across a MOP range
% Study period: October 1, 2022 to October 31, 2023
% Uses ALL available surveys (not just quarterly)
...
dateStart = datenum(2022, 10, 1);    % Start of study period
dateEnd = datenum(2023, 10, 31);     % End of study period
...
%% Find ALL survey dates (jetski/jumbo only) within date range
fprintf('\nSearching for ALL jetski/jumbo surveys in range %s to %s...\n', ...
...
% Filter for dates within the user-specified range
AllDates = AllDates(AllDates >= dateStart & AllDates <= dateEnd);
AllDates_sorted = sort(AllDates);

fprintf('Found %d surveys within date range\n', length(AllDates_sorted));

% Create survey pairs: each consecutive pair of surveys
SurveyPairs = [];
for i = 1:(length(AllDates_sorted)-1)
    SurveyPairs = [SurveyPairs; AllDates_sorted(i), AllDates_sorted(i+1)];
    fprintf('  Survey pair %d: %s to %s\n', i, ...
        datestr(AllDates_sorted(i), 'mmm dd, yyyy'), ...
        datestr(AllDates_sorted(i+1), 'mmm dd, yyyy'));
end

fprintf('Generated %d survey pairs\n', size(SurveyPairs, 1));
...
%% Loop through survey pairs and create plots
for q = 1:size(SurveyPairs, 1)
    SurveyDate1 = SurveyPairs(q, 1);
    SurveyDate2 = SurveyPairs(q, 2);
    
    fprintf('\n========================================\n');
    fprintf('Processing survey pair %d of %d\n', q, size(SurveyPairs, 1));
...
%% Collect all surveys from initial date to current survey date for profile plot
fprintf('Collecting surveys for profile panel...\n');

% Find all surveys from the first date in SurveyPairs to current SurveyDate2
FirstDate = SurveyPairs(1, 1);  % Initial survey date
AllSurveys = [FirstDate];  % Start with initial survey

% Add all end dates from pairs 1 through current pair q
for qq = 1:q
    if SurveyPairs(qq, 2) ~= FirstDate  % Don't duplicate
        AllSurveys = [AllSurveys; SurveyPairs(qq, 2)];
    end
end

% Sort chronologically
AllSurveys = sort(unique(AllSurveys));

fprintf('  Plotting %d surveys from %s to %s\n', length(AllSurveys), ...
    datestr(AllSurveys(1), 'mmm dd, yyyy'), ...
    datestr(AllSurveys(end), 'mmm dd, yyyy'));
...
% Define colors: preallocate based on maximum possible surveys
MaxPossibleSurveys = length(AllDates_sorted);  % All available surveys
col_all = jet(MaxPossibleSurveys);

% Map current surveys to the preallocated color scheme
nsurveys = length(AllSurveys);
% Find indices in the full survey list
color_indices = zeros(nsurveys, 1);
for ns = 1:nsurveys
    [~, sidx] = min(abs(AllDates_sorted - AllSurveys(ns)));
    color_indices(ns) = sidx;
end
...
if waveDataAvailable
    ...
    % Shade the period between SurveyDate1 and SurveyDate2 (pale red)
    ...
    % Plot dots for all surveys
    for ns = 1:length(AllSurveys)
        [~, tidx] = min(abs(timeW - AllSurveys(ns)));
        if ~isempty(tidx)
            if ns == 1 || ns == length(AllSurveys)
                % Bold dots for initial and final
                plot(ax_wave, AllSurveys(ns), Hs_monthly(tidx), 'k.', 'markersize', 15);
            else
                % Smaller dots for middle surveys
                plot(ax_wave, AllSurveys(ns), Hs_monthly(tidx), 'k.', 'markersize', 10);
            end
        end
    end
...
filename = fullfile(OutputDir, sprintf('ElevationChangeAndProfileMap_TBR23_M%d-%d_%s_to_%s.png', ...
    MopStart, MopEnd, dt1_str, dt2_str));
fprintf('Saving figure to: %s\n', filename);
print(gcf, filename, '-dpng', '-r150');
close(gcf);

end  % End of survey pair loop

fprintf('\n========================================\n');
fprintf('All survey pair plots complete!\n');
fprintf('Saved to: %s\n', OutputDir);
fprintf('========================================\n');
